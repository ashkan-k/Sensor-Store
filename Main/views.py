...
#                   'value': 'این یک متن تستی توضیحات درباره سایت در فوتر است'},
#     )[0].value

#     context['settings']['newsletter_text'] = Setting.objects.get_or_create(
#         key='newsletter_text',
#         defaults={'key': 'newsletter_text',
#                   'value': 'این یک متن تستی برای ثبت نام در خبرنامه است'},
#     )[0].value
...
#     context['filter_bar']['categories'] = Category.objects.filter(
...
#             price__gte=minPrice).filter(price__lte=maxPrice)
...
#             paginator = Paginator(
#                 context['products'], default_pagination_number)
#             if request.is_ajax():
#                 data_dict = paginate(
#                     paginator, page, sorting=context.get('sorting'))
...
#         context['most_visited_products'] = Product.objects.filter(
#             status=True).order_by('-viewCount')
#         context['new_products'] = Product.objects.filter(
#             status=True).order_by('-created_at')[:8]
#         context['most_liked_products'] = Product.objects.filter(
#             status=True).order_by('-likeCount')[:8]
#         context['sliders'] = Slider.objects.filter(
#             status=True).order_by('priority')
#         context['last_articles'] = Article.objects.filter(
#             status=1).order_by('-created_at')[:6]
...
#         context = StaticData()
#         context[
#             'contact_us_description'] = 'برای تماس با ما کافی است اطلاعات رو وارد کنید و متن درخواست خود را بنویسید و ارسال کنید و ما در اولین فرصت و نزدیک ترین زمان بررسی و رسیدگی میکنیم (یک متن تستی)'
...
#         context['related_products'] = context['product'].category.product_set.exclude(
#             slug=slug)
...
#         context['related_blogs'] = Article.objects.filter(
#             status=1).exclude(slug=slug)[:10]
...
#         data_dict = {"comments": html,
#                      'detail': 'نظر شما با موفقیت ثبت شد و بعد از تایید مدیر در سایت قرار میگیرد'}
#         return JsonResponse(data_dict, safe=False)
...
#                 product_id=request.POST['product_id'], user_id=request.user.id)
...
#                 product_id=request.POST['product'], user_id=request.user.id)
...
#         try:
#             notify_user = NotifyUser.objects.get(
#                 product_id=request.POST['product_id'], user_id=request.user.id)
#             notify_user.delete()
...
#     liked_disliked = product.likes_and_dislikes.filter(
#         user=user, type=type).first()
#     if liked_disliked:
#         liked_disliked.delete()


# def Create_Like_DisLike(product, user, type):
#     Like_And_DisLike.objects.create(
#         content_object=product, type=type, user=user)
...
#             liked = product.likes_and_dislikes.filter(
#                 user=request.user, type='like')
#             if liked:
#                 liked.delete()
#             else:
#                 Check_Like_DisLike_Exist(product, request.user, 'dislike')
#                 Create_Like_DisLike(product, request.user,
#                                     request.POST['type'])

#         else:
#             disliked = product.likes_and_dislikes.filter(
#                 user=request.user, type='dislike')
#             if disliked:
#                 disliked.delete()
#             else:
...
#         return render(request, "Layouts/front-master.html", {})
